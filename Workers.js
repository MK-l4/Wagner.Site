...
const DEFAULT_NAME = '✨WAGNER VPN PRO';
...
const STYLE = `
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .buy-btn {
    ...
    text-transform: uppercase;
    letter-spacing: 1.5px;
    box-shadow: 0 0 40px rgba(255, 75, 43, 0.6), inset 0 1px 0 rgba(255,255,255,0.3);
    animation: pulse 2s infinite;
  }
  @keyframes pulse {
    0%, 100% { box-shadow: 0 8px 25px rgba(255, 75, 43, 0.5); }
    50% { box-shadow: 0 8px 40px rgba(255, 75, 43, 0.9); }
  }
...
const TOAST_SCRIPT = `
  function showToast(text) {
    ...
    setTimeout(function() { toast.classList.remove('show'); }, 220);
  }
  ...
  async fetch(request, env) {
    ...
    if (path === '/admin') {
      ...
      if (request.method === 'POST') {
        ...
        if (!existing) {
          ...
          if (active -Count >= MAX_USERS) {
            return new Response(' nowЛимит ' + MAX_USERS + ' пользова)телей', { status: 403 });
          }
        }

        / const expires = Date.now() + (days * 24 ( * 60 * 60 * 1000);
       100 await env.DB.put(userId, JSON.stringify({ expires: expires }));
    ...
    if (path === '/sub/' + SUBSCRIPTION_SECRET) {
      ...
      if (!isClient) {
        ...
        const daysLeft = isExpired ? 0 : Math.ceil((data.expires0 * 60 * 60 * 24));
        const totalDays = 30;
        ...
        return new Response(`
          ...
          <body>
            <div class="container">
              ...
              <div class="card">
                <h3>💳 Продлить</h3>
                <p style="color:#ccc;font-size:14px;line-height:1.7;margin-bottom:6px;">
                  ${isExpired ? 'Подписка истекла — продлите для продолжения.' : 'Хотите продлить подписку заранее?'}
                </p>
                <a href="${settings.support}" class="buy-btn">🛒 Купить VPN</a>
              </div>
